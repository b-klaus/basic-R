...
% graphics.off();rm(list=ls());library('knitr');knit('EDA-lab.Rnw');  for(i in 1:2) system('R CMD pdflatex EDA-lab.tex');purl('EDA-lab.Rnw')
...
%setwd("/g/huber/users/klaus/uni/Teaching/EMBL-Teaching/Basic-R-Graphics/EDA-lab")
...
The package \CRANpkg{ggplot2} is one  of the most commonly used graphics packages for \R{}.
It is an alternative to the basic graphic system of \R{}, which is limited in various
aspects. In this lab we will use basic graphics in \R{}  and corresponding
 \CRANpkg{ggplot2} plots side by side.

\CRANpkg{ggplot2} is meant to be an implementation of the Grammar of Graphics, 
...
Thus, we have only created the data layer: first argument to \Rfunction{ggplot()}
is a data frame (it must be a data frame), and its second argument is 
\Rfunction{aes()}. You're never going to use \Rfunction{aes()} in any other 
context except for inside of other \CRANpkg{ggplot2} functions,
so it might be best not to think of \CRANpkg{aes()} as its own function, 
but rather as a special way of defining data--to--aesthetic mappings.

\subsection{Geometry layers}
...
There are a few things to take away from this step. 
First and foremost, the way you add new layers, of any kind, 
to a plot is with the \Rcode{+} operator. And, 
there's no need to only add them one at a time. You can string 
together any number of layers to add to a plot, separated by \Rcode{+}.

The next thing to notice is that all layers you add to a plot are, technically, functions. 
We didn't pass any arguments to \Rfunction{geom\textunderscore point()}, 
so the resulting plot represents the default behavior: solid black circular points.

If for no good reason at all we wanted to use a different point shape / color
in the plot, we could specify it inside of \Rfunction{geom\textunderscore point()}.
...
Finally, note that we didn't need to tell \Rfunction{geom\textunderscore point()}
about the $x$-- and $y$--axes. This may seem trivial, but it's a really important, 
and powerful aspect of \CRANpkg{ggplot2}. When you add any layer at all to a plot, 
it will inherit the data-to-aesthetic mappings which were defined in the data layer. 
...
Here we also clearly see the difference between \textbf{mapping} to and \textbf{setting} 
aesthetics. In the first plot, the shape and color are set to fixed values, while
in the second plot, they are determined by a grouping of the data. If you want
to set an aesthetic within \Rfunction{qplot}, you have to use the \Rcode{I()} operator:
(for ``treat as is'')
...
In any case, we clearly see that the  setosa plants have 
different Sepal.Length/Sepal.Width 
relationship compared to the other two species.
...
\subsection{Adding statistical transformations}
Following the geometry, the next layer you add is usually a statistical 
transformation.

We will explore one of these transformations by adding a regression line
to the data of each of the three plant species as a third layer. The 
semi--transparent ribbon surrounding the solid line is the 95\% confidence interval.
...
One important thing to realize is that it's not necessary to include 
the points in order to add a smoothing line. Here's what the plot would
look like with the  points omitted.
...
fits. You can obtain the statistical transformations by looking at the saved
...
\subsection{Summary of \CRANpkg{ggplot2} aesthetics} 


In \CRANpkg{ggplot2}, aesthetics are the graphical elements
which are mapped to data, and they are defined with \Rfunction{aes()}. 
To some extent, the aesthetics you need to define are dependent 
on the geometries you want to use, because line segments 
have different geometric properties than points, for example. However, 
there is also a great deal of uniformity in the aesthetics used across geometries. 
Here is a list of the most common aesthetics you'll want to define.



\begin{itemize}
    \item{\Rcode{x}} x--axis location

    \item{\Rcode{y}} y--axis location 
    
    \item{\Rcode{color}} The color of lines, points, and the outside borders of two dimensional 
    geometries (polygons, bars, etc.).
    
    \item{\Rcode{fill}}  The fill color of two dimensional geometries.
    
    \item{\Rcode{size}} The size of points, 
    or the weight of lines and borders of two dimensional geometries.
    
    \item{\Rcode{shape}} This is specific to points, and defines the point shape. 
    This is one of the few aesthetics to which you can't map a continuous variable.
    
    \item{\Rcode{linetype}} This defines the line type of any kind of line, path, or border 
    of a two dimensional geometry. This is another aesthetic 
    which cannot be mapped to a continuous variable.
    
    \item{\Rcode{alpha}}  This defines the opacity of any geometric property. It's less    
    commonly  mapped to data, and more often 
    hard coded to a single value as a solution for overplotting.
       
    \item{\Rcode{xend, yend}} You'll use these more rarely, usually when plotting a line 
    segment, or arrow. The beginning of the line segment will be located at x, y, 
    and the end of the line segment will be located at xend, yend.
    
    \item{\Rcode{ymin, ymax, (xmin, xmax)}}
    ymin and ymax are reserved for geometries which are devoted to representing
    ranges of data, like error bars, and ribbons. For the most part, these will 
    be expressed along the y-axis, but xmin and xmax are utilized for some geometries as 
    well.


The full documentation for \CRANpkg{ggplot2} can be found at \url{http://docs.ggplot2.org/current/}.

\end{itemize}


\subsection{Grouping and inheritance of aesthetics} 

When we added the regression lines to the iris scatterplot above, we saw that 
they were automatically split up by species. This is because subsequent layers 
added to the plot inherit aesthetics defined in the \Rfunction{ qplot() / ggplot()} 
data layer. However, we can override the inherited grouping by replacing 
it explicitly:
...
Note that geoms inherit aesthetic mappings from the \Rfunction{ qplot() / ggplot()}
data layer and not from any other layer.
...
Now, the lines that we added to the plot are colored by species, since
the geometry inherits the grouping from the data layer and not from the 
smoothing layer.


\subsection{Color in \CRANpkg{ggplot2}}

When you map a grouping variable to color, each point is  colored according to 
the group it belongs to. \CRANpkg{ggplot2} will automatically generate
a color palette of the right type and size, based on the data mapped to color, and 
create a legend on the side of the plot. 

As with everything, the specific color palette we use is adjustable. 
The default \CRANpkg{ggplot2} color palette is rather clever, however. 
Every color is  equidistant around an HSL color circle, 
and has equal luminance. The idea is that no category should be 
accidentally visually emphasized, but they can be hard for 
some colorblind readers, and they will all print to the same shade of grey.

If you don't like the default color scheme, there are other available 
color palettes, and you can define your own. One really nice set of color palettes 
comes from the package \CRANpkg{RColorBrewer}. You can explore the set of color 
palettes available in it \href{http://colorbrewer2.org/}{here}.

There are two very distinct kinds of color scales here: categorical and gradient. 
The use of one over the other has everything to do with the kind of data which is 
passed to color and fill, and they have their own specific customizations.

If you are particularly picky, there is also 
scale\textunderscore color\textunderscore manual(), 
where you define an arbitrary list of colors to use. Here 
are two examples of custom color definitions:
...
\subsection{An example: plotting experimental data}
...
map \Robject{min} to the x--axis and \Robject{Signal} to the y--axis and use a 
...
\CRANpkg{ggplot2} also allows you to easily split plots according to a factor 
...
\subsection{Gathering and spreading data frames}  
...
Thus, it was ``tidy'' and in a long format.
...
Our current data would is in a ``long'' format,
but we might want to transform it into a ``wide'' format, with a separate 
column for every time point. The package \CRANpkg{tidyr} allows 
you to do this easily. \CRANpkg{ggplot2} usually requires ``long' formats, which can be
obtained by using the function \Rfunction{gather}. Thus a  ``gathered''
data frame in \CRANpkg{ggplot2} corresponds to a ``long'' format.  ``wide'' formats
can be obtained using the function \Rfunction{spread}. As an example
...
variable, so we only include signal as a variable here and remove the Sigma
column. In some sense our data frame is not ``tidy enough'' making the removal
of additional value column necessary.
...
\CRANpkg{tidyr} has two main functions:
...
We can now ``gather'' the data frame again.  Again, we 
use \CRANpkg{tidyr} to achieve this.
We specify that we want to ``gather'' time columns again
...
adhesion. You can use the Bioconductor package \Biocpkg{biomaRt} to query
various genomic databases. The  code below was originally used to retrieve the
...
In \CRANpkg{ggplot2} we first  transform our data into an
appropriate \Robject{data.frame}. Then a barplot can be conveniently created 
via the \Rfunction{qplot} function that automatically picks a appropriate
geometry based on the input data.
...
Alternatively, we can map the bases to the x--axis and add binning by using
the bar geometry via the the function \Rfunction{ggplot}.
...
A pie--chart is easily created from the result of a table command in
base \R{}
...
In order to plot a pie chart with \CRANpkg{ggplot2}, we first need to create
a bar that is filled according to the number of bases. In order to achieve this 
we create a dummy factor for the x--axis and  map the binned base 
counts to the fill aesthetic.
...
\subsection{Scatterplots, stripcharts and beeswarms}
...
microarray data on two sub types of leukemia. We shall concentrate 
on the expression values of gene
"CCND3 Cyclin D3", which are collected in row 1042 of the data matrix
\Rfunction{golub}. To plot the data values one can simply use \Rfunction{plot(golub[1042,])}. 
In the resulting plot  the vertical axis gives the size of the expression
...
the data points a little bit. A nice alternative to the default stripcharts
is provided by the \CRANpkg{beeswarm}. The bee--swarm plot is a one-dimensional 
scatter plot like "stripchart", but with closely-packed, non-overlapping points. 
...
In \CRANpkg{ggplot2} these plots can be created
like this:
...
The ``jittering'' in \CRANpkg{ggplot2} occurs after the plot has been created
...
explicitly in \CRANpkg{ggplot2}.

A beeswarm can be produced by using the output of the beeswarm call 
which returns a data frame with the coordinates and rescaling the x–axis.
...
\Rfunction{qplot()} uses binning as the default geometry if only x--values are
...
estimate as returned by the \Rfunction{stat\_bin} 
statistical transformation used  by the ``geom\_ histogram'' 
geometry.
...
In \CRANpkg{ggplot2} a boxplot can be exactly as a stripchart,
...
In \CRANpkg{ggplot2} there exists a specialized statistic which performs
...
median. In \R{} it is computed by the
